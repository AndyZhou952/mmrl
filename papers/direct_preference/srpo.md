...
**Idea — Direct-Align**: Exploit the known noise prior. In flow matching, the forward process places the noisy state $x_t$ (interpolation at flow time $t \in [0,1]$, where $t=1$ is pure noise and $t=0$ is the clean image) on a linear path between the clean image $x_0$ and a fixed noise sample $\epsilon_\text{gt} \sim \mathcal{N}(0, I)$ — the **ground-truth noise prior**, drawn once per image and held constant:
...
This is the standard Tweedie recovery formula, but substitutes the **known** $\epsilon_\text{gt}$ instead of the network's prediction — making recovery exact and bypassing multi-step denoising entirely.

**Why this works**: The gradient $\partial r / \partial \theta$ (of the reward $r$ with respect to the trainable parameters $\theta$) passes through only **one network call** $v_\theta(x_t, t, c)$ — the velocity-prediction network conditioned on the encoded text prompt $c$. There is no rollout, no iterative denoising chain, no trajectory storage. The reward gradient flows back through a single velocity prediction, making this $T\times$ cheaper than full-chain backpropagation. Because $t$ is sampled uniformly over $[0,1]$, every noise level (including early high-noise steps) participates in training — all timesteps are aligned, not just the last few.
...
The discount down-weights the high-noise end of the trajectory, where one-step recovery is least accurate. The paper uses a general decaying schedule $\lambda(t)$; written concretely here as $\gamma^{T-t}$ with $\gamma \in (0,1)$ and $T$ the total step count, so high-noise timesteps get a smaller weight while low-noise ones are trusted more. The minus sign turns reward maximisation into a loss to minimise; $\hat{x}_0(x_t, \epsilon_\text{gt})$ is the one-step closed-form recovery above, written as a function of the noisy state and the known noise prior.

### Inversion regularisation

To prevent overfitting to the reward at late (low-noise) timesteps, SRPO adds an **inversion branch**, the regulariser $\mathcal{L}_\text{inv}$: instead of denoising, it re-injects noise (the inverted direction $+\epsilon_\text{gt}$ applied to the recovered image) and applies the *negative* preference there — a gradient descent that drives the model *away from* the negative condition $c_-$ on the renoised image. This penalises solutions that exploit the mapping for late $t$ without genuinely improving image quality. Because the penalty is decoupled across timesteps from the reward term, the regularisation acts as an implicit anchor without requiring a frozen reference model or KL term.
...
This is the Direct-Align loss $\mathcal{L}_\text{DA}$ with the absolute reward $r$ replaced by the semantic relative reward $r_\text{SRP}$, plus the inversion regulariser. To recap the symbols: the expectation averages over prompts $c$, flow time $t \sim \mathrm{Uniform}[0,1]$, and the fixed noise prior $\epsilon_\text{gt}$; $\gamma^{T-t}$ is the per-timestep discount ($\gamma \in (0,1)$, exponent counting steps to clean); $\hat{x}_0(x_t, \epsilon_\text{gt})$ is the one-step closed-form recovery; $r_\text{SRP}(\cdot, c) = f_\text{img}(\cdot)^\top(C_+ - C_-)$ is the positive-minus-negative-condition score; $\mathcal{L}_\text{inv}$ is the inversion-branch regulariser and $\lambda > 0$ its weight. No KL divergence. No frozen reference model. No SDE sampler required.
