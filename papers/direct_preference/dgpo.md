...
$$\boxed{
\mathcal{L}_\text{DGPO}(\theta) = -\mathbb{E}_{t,\epsilon}\left[\log\sigma\left(\beta\left(\sum_{i \in \mathcal{G}^{+}} w^{+}(i)\Delta\mathcal{L}_i - \sum_{j \in \mathcal{G}^{-}} w^{-}(j)\Delta\mathcal{L}_j\right)\right)\right]
}$$
...
2. Independently sampling a timestep $t \sim \mathrm{Uniform}\lbrace{}t_\min, \ldots, T\rbrace$ (with a floor $t_\min > 0$ that excludes the lowest-noise steps, where the ELBO proxy is unreliable) and fresh noise $\epsilon^{(i)} \sim \mathcal{N}(0,I)$.
