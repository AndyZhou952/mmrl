...
AWM addresses a conceptual gap that prior work overlooked: the pretraining objective (flow matching MSE) and the RL objective (GRPO / DDPO) are structurally different losses. AWM establishes that this gap is unnecessary — the pretraining loss can be extended to an RL objective by a single modification: multiplying by the group-relative advantage. The result is a method that is maximally compatible with the pretraining optimiser, requires no SDE, and is the **clean-target form of DDPO's implicit (noisy-target) objective** — the paper shows DDPO's policy gradient is implicitly matching against a *noisy* target, and AWM corrects it to the clean velocity target, removing that variance.

---

## Problem — DDPO (and FlowGRPO) implicitly use a noisy target that diverges from pretraining

**Issue**: DDPO's policy gradient update at timestep $t$ is proportional to the reward times the gradient of $\log \pi_\theta(x_{t-1} \mid x_t, c)$. Here $t \in [0,1]$ is the continuous flow time ($t=1$ pure noise, $t=0$ clean), $c$ is the conditioning prompt, $r = r(x_0, c)$ is the scalar reward on the final image, $x_t$ is the noisy state, and $v_\theta(x_t, t, c)$ is the model's velocity prediction. For a Gaussian policy, this is equivalent to:
...
The target $(x_{t-1} - \epsilon)$ — with $\epsilon \sim \mathcal{N}(0, I)$ the Gaussian noise and $x_{t-1}$ the next denoised state — is a *stochastic* sample from the policy at step $t-1$. Compare this to the pretraining flow matching loss, which uses the **clean** velocity target $u_t = x_0 - \epsilon$ (the constant velocity along the straight-line path $x_t = (1-t)x_0 + t\epsilon$ that connects clean image $x_0$ to noise $\epsilon$):
...
Here $N$ is the group size (images generated per prompt $c$); $i$ indexes the group, so $x_t^{(i)}$, $u_t^{(i)} = x_0^{(i)} - \epsilon^{(i)}$, and $\hat{A}^{(i)}$ are the noisy state, clean target, and advantage of sample $i$; $\hat{A}^{(i)}$ is the group-relative advantage (the GRPO-normalised reward, defined below); and $w(t) \geq 0$ is a per-timestep weight. Note there is **no importance ratio** $\rho_t = \pi_\theta / \pi_{\theta_\text{old}}$ here: the advantage simply *reweights a matching loss*, it does not multiply a likelihood ratio.

**Why this works**: The clean target eliminates the noise residual variance completely. The advantage weighting provides the RL signal: positive-advantage samples push $v_\theta$ toward the target (reinforcing the behaviour), while negative-advantage samples push $v_\theta$ away from the target (suppressing the behaviour). Crucially, this is the **exact same loss** as flow matching pretraining — but with advantage weighting added, so the RL update stays aligned with the pretraining objective and optimiser. No architectural changes, no SDE, no importance ratio.

**Result**: AWM **matches FlowGRPO on GenEval (SD3.5-M) at ~8× speedup**, and reaches up to **~24× on the OCR task** (its largest speedup across benchmarks), with comparable-or-better PickScore on FLUX — the clean target removes the noisy-IS-loss variance without an SDE or reference lookup. See the consolidated [Results](#results) table below.
...
|---|---|---|
| LLMs | $\mathcal{L}_\text{SFT} = -\mathbb{E}[\log\pi(y\mid x)]$ | $\mathcal{L}_\text{PPO} = -\mathbb{E}[\hat{A}\cdot\log\pi(y\mid x)]$ |
...
The group-relative advantage — the per-prompt reward standardised within its group of $N$ samples:

$$\hat{A}^{(i)} = \frac{r^{(i)} - \overline{r}}{\mathrm{std}(\lbrace{}r^{(j)}\rbrace) + \delta}, \quad r^{(i)} = r(x_0^{(i)}, c)$$

where $r^{(i)}$ is the reward of image $i$, $\overline{r} = \mathrm{mean}(\lbrace r^{(j)}\rbrace_{j=1}^N)$ is the group mean, and $\delta > 0$ is a small stability constant.
...
where the network input $(1-t)x_0^{(i)} + t\epsilon^{(i)}$ is the straight-line interpolant $x_t^{(i)}$ between clean image $x_0^{(i)}$ and per-image noise $\epsilon^{(i)} \sim \mathcal{N}(0, I)$, the regression target $x_0^{(i)} - \epsilon^{(i)}$ is the clean velocity $u_t^{(i)}$, and $w(t) \geq 0$ is a timestep weighting schedule (default: $w(t) = 1$, matching the pretraining schedule). No importance ratio, no SDE, no reference policy during the gradient step.
...
|---|---|---|
| SD3.5-M | GenEval | Matches FlowGRPO at **~8× speedup** |
| SD3.5-M | OCR | Matches FlowGRPO at **~24× speedup** (largest across benchmarks) |
