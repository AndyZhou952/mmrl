...
**Idea**: Confine both SDE sampling and gradient tracking to a **contiguous sliding window** $\mathcal{W}(l) = \lbrace{}l, l{+}1, \ldots, l{+}w{-}1\rbrace$ of $w$ denoising steps — here $l$ is the window's left boundary (its starting step index) and $w = |\mathcal{W}|$ is the fixed window size (number of optimised steps). Outside the window, use a standard deterministic ODE with no gradient tracking. Slide the window through the trajectory over training iterations (advancing $l$), so different segments accumulate gradient updates over time. The paper's intuition: early denoising steps shape global structure and later steps fine detail, so the window enacts a curriculum that optimises high-impact early steps first.

**Why this works**: The GRPO importance ratio $\rho_t$ (per-step ratio of the new vs. old policy's transition density at step $t$; defined below) is meaningful only at SDE steps (deterministic ODE steps have $\rho_t = 1$ trivially, contributing nothing to the gradient). Confining SDE to $w$ steps reduces memory to $O(w \cdot d)$, and allows the non-window steps to use fast ODE solvers — improving trajectory quality at the tail (high detail) and head (coarse structure). Sliding the window ensures the entire trajectory is eventually covered across iterations, so the full reward landscape is optimised even though only $w$ steps are updated at each iteration.
...
For a trajectory with $T$ denoising steps, window starting at step $l$. Here $x_t$ is the noisy latent at flow time $t$, with $x_T$ (equivalently $x_1$) pure Gaussian noise and $x_0$ the clean image; the trajectory runs $x_T \to \cdots \to x_0$:
...
**Outside window** (ODE, no grad), where $v_\theta(x_t, t, c)$ is the model's velocity prediction conditioned on prompt $c$ and $\Delta t$ is the step size:

$$x_{t-\Delta t} = x_t - v_\theta(x_t, t, c)\Delta t$$

**Inside window** (SDE with score correction, with grad), where $\sigma_t$ is the SDE diffusion (noise) coefficient at step $t$ and $\epsilon_t$ is freshly sampled Gaussian noise:
...
where $\hat{x}_0 = x_t - tv_\theta(x_t,t,c)$ is the Tweedie estimate of the clean image. The per-step transition inside the window is Gaussian, with mean $\mu_\theta(x_t,t,c)$ equal to the deterministic part of the update above:
...
**Window sliding** — every $\tau$ training iterations the left boundary advances by stride $s$ (clamped so the window stays inside the trajectory):
...
**Idea — MixGRPO-Flash**: Shrink the window to $w=1$ step and replace all ODE steps with **DPM-Solver++** (a high-order ODE solver) at compression ratio $\tilde{r} < 1$ — the fraction of ODE steps retained after solver compression:
...
GRPO-clip loss restricted to window steps only; ODE steps contribute nothing. The outer average is over the group of $N_g$ samples generated per prompt (group size), and the inner average is over the $|\mathcal{W}| = w$ steps in the window:
...
Here $\hat{A}^{(i)}$ is the group-relative advantage of sample $i$ — its reward standardised within the group, $\hat{A}^{(i)} = (r^{(i)} - \mathrm{mean}(r)) / \mathrm{std}(r)$ — and $\epsilon$ is the PPO clip range (the clip caps the ratio to $[1{-}\epsilon, 1{+}\epsilon]$ so a single update cannot move the policy too far).

Importance ratio $\rho_t^{(i)}$ (window steps only) — the ratio of the current policy $\pi_\theta$ to the rollout policy $\pi_{\theta_\text{old}}$ for taking the realised transition $x_{t-\Delta t}^{(i)}$, written out from the Gaussian log-densities (means $\mu_\theta$, $\mu_{\theta_\text{old}}$ under current and frozen rollout parameters):
