...
$$\mathcal{G}^{+} = \lbracei : \hat{A}^{(i)} > 0\rbrace, \quad \mathcal{G}^{-} = \lbracei : \hat{A}^{(i)} \leq 0\rbrace$$
...
2. Independently sampling $t \sim \mathrm{Uniform}\lbracet_\min, T\rbrace$ and $\epsilon^{(i)} \sim \mathcal{N}(0,I)$.
