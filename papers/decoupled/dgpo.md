...
> Notation: follows [NOTATION.md](../NOTATION.md) §6 (ELBO shorthand). DDPM convention: $\epsilon_\theta$, $x_t = \sqrt{\bar\alpha_t}x_0 + \sigma_t\epsilon$; the method also applies to flow matching with an analogous MSE substitution. Positive group $\mathcal{G}^{+}$, negative group $\mathcal{G}^{-}$; advantage weights $w^+(i)$, $w^-(j)$.
...
Via the diffusion ELBO (see [NOTATION.md §6](../NOTATION.md)):

$$\log\frac{p_\theta(x_0|c)}{p_{\theta_\text{ref}}(x_0|c)} \approx -\mathbb{E}_t\!\left[\underbrace{\|\epsilon_\theta(x_t,t,c) - \epsilon\|^2}_{\mathcal{L}_\theta(x_0)} - \underbrace{\|\epsilon_{\theta_\text{ref}}(x_t,t,c) - \epsilon\|^2}_{\mathcal{L}_{\theta_\text{ref}}(x_0)}\right]$$
...
2. Independently sampling $t \sim \mathrm{Uniform}\{t_\min, T\}$ and $\epsilon^{(i)} \sim \mathcal{N}(0,I)$.
