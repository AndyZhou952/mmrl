...
Almost every algorithm in this survey fine-tunes a **flow matching** image/video model. Three things are worth holding onto: what the model *is*, how it is *trained and sampled*, and the **one structural fact the entire field is built around** — the sampler is a deterministic ODE with *no per-step probability*, which is exactly the thing RL needs.
...
1. **What a flow model is** — a learned velocity field, read as driving directions from noise to data.
2. **How it is trained** — regress that velocity against a straight-line displacement target.
3. **How it is sampled** — integrate the velocity as an ODE from noise down to a clean latent.
4. **The crux for RL** — that ODE is deterministic, so a denoising step has *no probability* to compute a policy ratio from.
5. **The two escapes** — convert the ODE to an SDE (policy-gradient family) or sidestep per-step densities entirely (direct-preference family) — plus a one-paragraph DDPM note for the few pages that need it.
...
A couple of methods touch the older **DDPM** parameterisation: a noise-prediction network $\epsilon_\theta$ and a curved forward process $x_t = \sqrt{\bar\alpha_t}x_0 + \sigma_t\epsilon$. You only need it for DanceGRPO's DDPM branch and the ELBO used by DGPO / the Diffusion-DPO precursor, and those pages define the symbols where they use them. Mentally, DDPM is "the same idea on a curvier path." Everything else here is flow matching.
