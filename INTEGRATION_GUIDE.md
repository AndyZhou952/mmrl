...
### 4.2 Notation: follow NOTATION.md, but be self-contained for the reader

There is **no `> Notation:` header** on a page, and a page **never references `NOTATION.md`, its section numbers, or "the unified notation" in reader-facing prose.** NOTATION.md is an *authoring-and-CI* contract, not something the reader is sent to. Two rules:

1. **Globally consistent (author + CI).** Default every symbol to NOTATION.md. If a symbol is defined there, use it exactly as defined, even if the paper uses a different letter. This is verified automatically by `.github/scripts/check_notation.py` (wired into the monthly workflow on the integration PR), which flags any canonical symbol reused with a different meaning.

2. **Self-contained (reader).** Wherever a symbol first appears on the page, gloss it inline in the prose right there — even symbols that are in NOTATION.md — so a first-time visitor can follow the math without leaving the page (e.g. define $\rho_t$ and $\hat{A}^{(i)}$ immediately after they first appear). For a page that adopts a non-default convention (e.g. DDPM/score-based instead of flow matching, as in Diffusion-DPO and DGPO), introduce that convention as one short woven sentence in the Context or first Problem section — not as a labelled "Notation" block.

Any symbol genuinely not covered by NOTATION.md (a paper-specific stochasticity parameter, a local group $\mathcal{G}^{+}$, a keep-mask $m_t$, …) is simply defined inline at first use like any other. If such a symbol will recur across multiple papers, also add it to NOTATION.md (see §5).
...
2. **If the paper uses a different symbol**, use the NOTATION.md symbol in the body instead — do not introduce the paper's symbol. (You may note the paper's choice once, inline, e.g. "(the paper writes $X$)".)
3. **If the paper introduces a genuinely new symbol** (not covered in NOTATION.md), define it inline at first use (§4.2). If the symbol will be used across multiple papers, add it to NOTATION.md under the appropriate section.
...
Is the symbol used in more than one paper, or likely to appear in future papers?
  YES → Add it to NOTATION.md under the appropriate section (and still define it
        inline at first use on the page)
  NO  → Define it inline at first use in the page body only

Does the new symbol conflict with an existing NOTATION.md symbol?
  YES → Use the NOTATION.md symbol in the body; if it helps, note the paper's
        choice once inline: "(the paper writes X)"
  NO  → Introduce the new symbol with its definition on first use in the body
```

In all cases the definition lives **inline in the prose** at first use — never in a
`> Notation:` header (there is none) and never via a reference to NOTATION.md.
...
- [ ] **Notation**: every canonical symbol matches NOTATION.md (CI `check_notation.py` will verify); every symbol is also glossed inline at first use so the page is self-contained; no `> Notation:` header and no NOTATION.md reference in reader-facing prose
