...
6. Literal | inside inline math on a table row — the pipe-table parser runs
   before KaTeX, so a | typed directly into a $...$ span on a table row is
   read as a column separator and splits the cell mid-formula (e.g.
   $\\log\\pi(y|x)$). Use \\mid (conditioning) or \\lvert..\\rvert / \\Vert{}.
...
# Greek (and variant) math macros that, when glued directly to a following
# ASCII letter, form an undefined control sequence KaTeX cannot render (e.g.
# \betaD, \thetadt, \betav). Ordered longest-first so the alternation matches
# the full macro name (e.g. \varepsilon as `varepsilon`, not `var`+`epsilon`).
GREEK_MACROS = [
    'epsilon', 'varepsilon', 'vartheta', 'varsigma', 'upsilon', 'varphi',
    'varpi', 'varrho', 'lambda', 'Lambda', 'Upsilon', 'Sigma', 'Theta',
    'Gamma', 'Delta', 'Omega', 'sigma', 'alpha', 'gamma', 'delta', 'theta',
    'kappa', 'omega', 'beta', 'zeta', 'iota', 'rho', 'tau', 'phi', 'chi',
    'psi', 'eta', 'mu', 'nu', 'xi', 'pi', 'Xi', 'Pi', 'Phi', 'Psi',
]
# Longest-first to avoid partial matches (e.g. \var of \varepsilon).
_GREEK_ALT = '|'.join(sorted(GREEK_MACROS, key=len, reverse=True))
GREEK_GLUED_RE = re.compile(r'\\(?:' + _GREEK_ALT + r')(?=[A-Za-z])')
...
CHECKS = [
    ...
    (
        GREEK_GLUED_RE.pattern,
        'ERROR',
        r'Greek macro glued to a following letter (e.g. \\betaD, \\thetadt): '
        r'KaTeX reads the whole run as one undefined control sequence and emits '
        r'an empty/broken box. Add a space, e.g. \\beta D.',
    ),
    (
        r'\^[\*\+](?![{a-zA-Z0-9])',
        'ERROR',
...
def structural_issues(filepath: str, text: str) -> list:
    ...
    for i, line in enumerate(lines):
        ...
        # Literal | inside inline math on a Markdown table row. The pipe-table
        # parser runs before KaTeX and reads the | as a column separator, so the
        # cell is split mid-formula (e.g. $\log\pi(y|x)$ becomes "$\log\pi(y" + "x)$").
        # Note this is a |-character typed directly into math, distinct from the
        # \| escape sequence handled by the per-span ERROR above.
        if line.lstrip().startswith('|'):
            for m in re.finditer(r'(?<!\$)\$([^$\n]{1,200}?)\$(?!\$)', line):
                if re.search(r'(?<!\\)\|', m.group(1)):
                    issues.append((
                        filepath, i + 1, 'ERROR',
                        'Literal | inside inline math on a table row: the Markdown '
                        'pipe-table parser reads it as a column separator and splits '
                        r'the cell, breaking the formula. Use \mid (conditioning) or '
                        r'\lvert … \rvert / \Vert{} (norm/abs) instead of a bare |.',
                        line.strip()[:100],
                    ))
...
def _fix_math_span(math: str) -> str:
    ...
    # Greek macro glued to a following letter (\betaD → \beta D): insert a
    # single space to terminate the command name. Always safe for this class.
    math = GREEK_GLUED_RE.sub(lambda m: m.group(0) + ' ', math)
    return math
